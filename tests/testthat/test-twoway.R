...
test_that("twoway creates a table1 object", {
  ...
  tbl <- twoway(var1 = gender, var2 = group, data = df)
...
test_that("twoway works with var2lab specified", {
  ...
  expect_silent(twoway(var1 = gender, var2 = group, data = df, var2lab = "Group Label"))
})
...
test_that("twoway works with existing label", {
  ...
  expect_silent(twoway(var1 = gender, var2 = group, data = df))
})

test_that("twoway fails if var not in data", {
  df <- data.frame(gender = c("Male", "Female"))
  expect_error(twoway(var1 = gender, var2 = group, data = df))
})
