...
test_that("sumfun errors with NAs if na.rm = FALSE", {
  x <- c(1, 2, NA, 4)
  expect_error(suppressWarnings(sumfun(x, na.rm = FALSE)), "missing values and NaN's not allowed")
})

test_that("sumfun is deprecated", {
  lifecycle::expect_deprecated(sumfun(c(1, 2, 3, 4)))
})
